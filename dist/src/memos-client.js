...
function findDotEnv(explicitFile) {
    if (explicitFile) {
...
function resolveConfiguredPath(value, baseDir) {
    if (!value)
        return path.resolve("./.data/local-memos.json");
...
export class MemosClient {
    ...
    async createMemo(content, visibility = "PRIVATE") {
        if (this.mode === "local-json")
            return this.#createLocal(content, visibility);
    ...
    async updateMemo(memo, content) {
        if (this.mode === "local-json")
            return this.#updateLocal(memo, content);
        const name = memo.name || (memo.id ? `memos/${memo.id}` : "");
        if (!name)
            throw new Error("Cannot update memo without a name or id.");
    ...
    #updateLocal(memo, content) {
        const id = memoIdFromName(memo.name) || String(memo.id || "");
        if (!id)
            throw new Error("Cannot update local memo without a name or id.");
        const items = this.#readLocal();
        const index = items.findIndex((item) => memoIdFromName(item.name) === id || String(item.id || "") === id);
        if (index < 0)
            throw new Error(`Cannot update missing local memo ${id}.`);
        const updated = {
            ...items[index],
            content,
            updateTime: nowIso()
